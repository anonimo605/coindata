...
const InvestDialog = ({ plan, wallet, onInvested }: { plan: InvestmentPlan, wallet: UserWallet | undefined, onInvested: () => void }) => {
  ...
  const investmentAmount = isFixedPrice ? (plan.minInvestment ?? 0) : (parseFloat(amount) || 0);

  const dailyEarning = investmentAmount * (plan.dailyReturnPercentage / 100);
  ...
  const handleInvest = async () => {
    if (!user || !firestore || !wallet || !investmentAmount || investmentAmount <= 0) {
      toast({ variant: 'destructive', title: 'Error', description: 'El plan no tiene un precio válido.' });
      return;
    }
  ...
  return (
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Tu saldo actual es de {formatCurrency(wallet?.balance ?? 0, { currency: 'USD' })}.
        </DialogDescription>
      </DialogHeader>
        ...
        {isFixedPrice ? (
             <div className="space-y-2">
                ...
                <Input 
                    readOnly 
                    disabled 
                    value={formatCurrency(investmentAmount, { currency: 'USD' })} 
                    className="text-lg font-bold text-foreground"
                />
            </div>
        ) : (
             <div className="space-y-2">
                <Label htmlFor="investment-amount">Monto a Invertir (USD)</Label>
                <Input
            ...
            <Card className="bg-muted">
                ...
                <CardContent className="p-4 pt-2 space-y-2 text-sm">
                    <div className="flex justify-between">
                        ...
                        <span className="font-bold text-green-600">{formatCurrency(dailyEarning, { currency: 'USD' })}</span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-muted-foreground">Ganancia Semanal</span>
                        <span className="font-bold text-green-600">{formatCurrency(weeklyEarning, { currency: 'USD' })}</span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-muted-foreground">Ganancia Mensual (30 días)</span>
                        <span className="font-bold text-green-600">{formatCurrency(monthlyEarning, { currency: 'USD' })}</span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-muted-foreground">Ganancia Total</span>
                        <span className="font-bold text-green-600">{formatCurrency(totalEarning, { currency: 'USD' })}</span>
                    </div>
                </CardContent>
            </Card>
        )}
        <Button onClick={handleInvest} disabled={isLoading || investmentAmount <= 0} className="w-full">
          {isLoading ? 'Procesando...' : `Confirmar Inversión por ${formatCurrency(investmentAmount, { currency: 'USD' })}`}
        </Button>
      </div>
    </DialogContent>
  );
...
export default function PlansPage() {
    ...
    const getPlanLimitText = (plan: InvestmentPlan) => {
        if (plan.minInvestment && plan.maxInvestment && plan.minInvestment === plan.maxInvestment) {
            return formatCurrency(plan.minInvestment, { currency: 'USD', minimumFractionDigits: 0 });
        }
        if (plan.minInvestment && plan.maxInvestment) {
          return `${formatCurrency(plan.minInvestment, {currency: 'USD', minimumFractionDigits: 0})} - ${formatCurrency(plan.maxInvestment, { currency: 'USD', minimumFractionDigits: 0})}`;
        }
        if (plan.minInvestment) {
          return `Desde ${formatCurrency(plan.minInvestment, { currency: 'USD', minimumFractionDigits: 0})}`;
        }
        if (plan.maxInvestment) {
          return `Hasta ${formatCurrency(plan.maxInvestment, { currency: 'USD', minimumFractionDigits: 0})}`;
        }
        return 'Flexible';
    }
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
              {availablePlans?.map((plan) => {
                const isFixedPrice = plan.minInvestment && plan.maxInvestment && plan.minInvestment === plan.maxInvestment;
                const investmentAmount = plan.minInvestment ?? 0;
                const dailyEarning = investmentAmount * (plan.dailyReturnPercentage / 100);
                const weeklyEarning = dailyEarning * 7;
                const monthlyEarning = dailyEarning * 30;
                const totalEarning = dailyEarning * plan.durationDays;
                        ...
                        <Card onClick={() => handlePlanClick(plan)} className="cursor-pointer overflow-hidden group transition-all hover:shadow-xl hover:-translate-y-1 rounded-lg flex flex-col border-2 border-primary">
                            <CardContent className="p-3 space-y-3 flex flex-col flex-grow">
                                ...
                                <div className="border rounded-lg p-3 space-y-2 flex-grow">
                                    ...
                                    {isFixedPrice && investmentAmount > 0 && (
                                        <div className="pt-2">
